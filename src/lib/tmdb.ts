...
export async function searchShows(query: string) {
  ...
  const res = await fetch(
    `${TMDB_BASE}/search/tv?query=${encodeURIComponent(query)}&api_key=${key}`
  );
...
export async function fetchAllEpisodesFromTMDB(showId: number): Promise<TMDBEpisode[]> {
  const key = requireTmdbKey();
  const showResp = await fetch(`https://api.themoviedb.org/3/tv/${showId}?api_key=${key}`);
  ...
  for (const season of showJson.seasons) {
    const seasonResp = await fetch(
      `https://api.themoviedb.org/3/tv/${showId}/season/${season.season_number}?api_key=${key}`
    );
